...
class ReviewSerializerTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.serializer_data = {
            "user": self.user.email,  # Expecting the username as the read-only field
    ...
    def test_review_serializer_create(self):
        ...
        duplicate_review = Review.objects.filter(user=self.user, product=self.product).count()
        self.assertEqual(duplicate_review, 1, "Duplicate review exists for this user-product combination.")
    ...
    def test_review_serializer_invalid_rating(self):
        ...
        self.assertIn('rating', serializer.errors)  # The rating should raise a validation error
