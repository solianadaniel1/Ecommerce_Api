...
class ReviewViewSetTest(APITestCase):
    ...
    def setUp(self):
        ...
        """
        # Create test users
        self.user1 = get_user_model().objects.create_user(username='user1', password='password123',email="user1@example.com" )
        self.user2 = get_user_model().objects.create_user(username='user2', password='password222', email="user2@example.com")
        
        # Create a test product
        self.product = Product.objects.create(name="Test Product", price=100.00, stock_quantity=50)
    ...
    def test_create_review_authenticated(self):
        ...
        """
        url = '/api/reviews/'
        response = self.client.post(url, self.review_data, HTTP_AUTHORIZATION=f'Bearer {self.token_user1}')
    ...
    def test_create_duplicate_review(self):
        ...
        url = '/api/reviews/'
        self.client.post(url, self.review_data, HTTP_AUTHORIZATION=f'Bearer {self.token_user1}')
    ...
    def test_update_review(self):
        ...
        response = self.client.patch(url, update_data, HTTP_AUTHORIZATION=f'Bearer {self.token_user1}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_other_user_review(self):
        ...
        url = f'/api/reviews/{review_id}/'
        response = self.client.put(url, update_data, HTTP_AUTHORIZATION=f'Bearer {self.token_user2}')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
