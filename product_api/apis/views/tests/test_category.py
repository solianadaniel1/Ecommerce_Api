...
class CategoryViewSetTest(APITestCase):
    def setUp(self):
        ...
        self.category2 = Category.objects.create(
            name="Clothing", parent_category=self.category1
        )
    ...
    def test_create_category_admin(self):
        ...
        response = self.client.post(
            url, data, HTTP_AUTHORIZATION=f"Bearer {self.admin_token}"
        )
        ...
        )  # Ensure the category count has increased
    ...
    def test_create_category_regular_user(self):
        ...
        response = self.client.post(
            url, data, HTTP_AUTHORIZATION=f"Bearer {self.user_token}"
        )
    ...
    def test_search_categories(self):
        ...
        url = reverse("category-list")
        response = self.client.get(
            url, {"search": "Clothing"}, HTTP_AUTHORIZATION=f"Bearer {self.admin_token}"
        )
    ...
    def test_ordering_categories(self):
        ...
        url = reverse("category-list")
        response = self.client.get(
            url, {"ordering": "name"}, HTTP_AUTHORIZATION=f"Bearer {self.admin_token}"
        )
        print(response.data)
